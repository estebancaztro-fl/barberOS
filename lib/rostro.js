 ...
 * Súbela cada vez que edites los textos de FORMAS. Las fichas guardan la
 * versión con la que fueron asesoradas, así se sabe cuáles quedaron atrás.
 */
export const VERSION_CATALOGO = "2026-08";

/**
 * Copia fija de la recomendación en el momento de darla.
 * Queda como constancia de lo que se le dijo al cliente ese día, aunque
 * después cambiemos los textos del catálogo.
 */
export function copiarRecomendacion(forma) {
...
/** Registro de visagismo listo para guardar en la ficha del cliente */
export function registroVisagismo({ forma, similitud = null, confianza = null, origen, fecha }) {
  return {
    forma, similitud, confianza, origen, fecha,
    recomendacion: copiarRecomendacion(forma),
  };
}

/** ¿La ficha quedó con recomendaciones de una versión anterior? */
export function estaDesactualizada(visagismo) {
  return !!visagismo?.recomendacion && visagismo.recomendacion.version !== VERSION_CATALOGO;
}
